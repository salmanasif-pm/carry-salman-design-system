...
for (const f of files) {
  ...
  for (const re of BANNED) { const m = t.match(re); if (m) fails.push(`[restricted-content] ${f} :: ${m[0]}`); }
  for (const re of CLAIMS_BANNED) { const m = t.match(re); if (m) fails.push(`[claims-register-banned] ${f} :: ${m[0]}`); }
  for (const re of PLACEHOLDER) { const m = t.match(re); if (m) fails.push(`[placeholder-identity] ${f} :: ${m[0]}`); }
