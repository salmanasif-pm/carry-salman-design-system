...
/** wording='plain' renders a legitimately empty cell as the word "none" instead of an em dash (public renders; the dash is on the banned list). */
export function Matrix({columns=[], rows=[], caption, firstColWidth='minmax(120px,.8fr)', wording='system', style}) {
  ...
  return <div data-carry-matrix style={{overflowX:'auto',...style}}>
    ...
    <div role="table" style={{display:'grid',gridTemplateColumns:`${firstColWidth} repeat(${columns.length},minmax(96px,1fr))`}}>
      ...
      {rows.map((r,ri)=><React.Fragment key={ri}>
        ...
        {r.cells.map((c,ci)=>{const o=typeof c==='object'&&c!==null?c:{value:c};return <div key={ci} role="cell" style={{...cellStyle,background:r.emphasis?'var(--warmth)':'transparent',display:'flex',gap:'var(--sp-3)',alignItems:'flex-start'}}>{(o.certainty||o.state||o.gated) && <StateMark certainty={o.certainty} state={o.state} gated={o.gated ?? (o.gate?{gate:o.gate}:undefined)} wording={wording} showLabel={false} size={11} style={{width:'0.75em',height:'0.75em'}}/>}<span style={o.value==='—'?{color:'var(--ink-4)'}:null}>{o.value==='—'&&wording==='plain'?'none':o.value}</span></div>;})}
